...
class AuthorizationAjaxRequest extends AjaxRequest
{
    ...
    public function login()
    {
        ...
        if (empty($username)) {
            $this->setFieldError("username", "Введите никнейм");
            return;
        }
        ...
        if (!$auth_result) {
            $this->setFieldError("password", "Неправильный никнейм или пароль");
            return;
        }

        $this->status = "ok";
        $this->setResponse("redirect", ".");
        $this->message = sprintf("Здравствуйте, %s! Доступ разрешен.", $username);
    }
    ...
    public function register()
    {
		...
		$name1 = $this->getRequestParam("name1");
		$surname = $this->getRequestParam("surname");
    $patr = $this->getRequestParam("patr");
		...
		if (empty($name1)) {
            $this->setFieldError("name1", "Введите имя");
            return;
        }
		if (empty($surname)) {
            $this->setFieldError("surname", "Введите фамлию");
            return;
        }
    if (empty($patr)) {
            $this->setFieldError("patr", "Введите отчество");
            return;
        }
        ...
        try {
            $new_user_id = $user->create($username, $password1, $email, $name1, $surname, $patr);
        } catch (\Exception $e) {
        ...
        $this->message = sprintf("Здравствуйте, %s! Спасибо за регистрацию.", $username);
        $this->setResponse("redirect", "/");
        $this->status = "ok";
    }
}
